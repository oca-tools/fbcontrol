...
<div class="saas-hero-card reports-hero split-full">
    ...
    <form class="row g-3 saas-filter-grid mt-3" method="get" action="/" data-ajax-filter data-ajax-target=".app-content">
        <input type="hidden" name="r" value="relatorios/consulta">
        ...
        <div class="col-12 saas-toolbar">
            ...
            <a class="btn btn-primary btn-xl" href="/?r=relatorios/consulta" data-ajax-link data-ajax-target=".app-content">Remover filtro</a>
        </div>
    </form>
    </details>
</div>
            ...
            <ul class="pagination pagination-sm mb-0">
                <?php foreach ($paginationPages($mapPage, $mapTotalPages) as $i): ?>
                    <?php if ($i === null): ?>
                        ...
                        <?php continue; ?>
                    <?php endif; ?>
                    <?php $mapQuery = http_build_query(array_merge($filters, ['r' => 'relatorios/consulta', 'map_page' => $i, 'bi_page' => $biPage])); ?>
...
<div class="card p-4">
    ...
    <form class="row g-2 align-items-end mb-3" method="get" action="/" data-ajax-filter data-ajax-target=".app-content" data-ajax-preserve-scroll="1">
        ...
        <div class="col-12 col-md-4 d-flex gap-2">
            ...
            <a class="btn btn-outline-secondary flex-fill" href="/?<?= h(http_build_query(array_merge($filters, ['r' => 'relatorios/consulta']))) ?>" data-ajax-link data-ajax-target=".app-content" data-ajax-preserve-scroll="1">Limpar BI</a>
        </div>
    </form>
    <div class="table-responsive">
        <table class="table table-sm align-middle" data-no-auto-pagination="1">
            ...
            <ul class="pagination pagination-sm mb-0">
                <?php foreach ($paginationPages($biPage, $biTotalPages) as $i): ?>
                    <?php if ($i === null): ?>
                        ...
                        <?php continue; ?>
                    <?php endif; ?>
                    <?php $biQuery = http_build_query(array_merge($filters, ['r' => 'relatorios/consulta', 'bi_restaurante_id' => $biFilters['restaurante_id'] ?? '', 'bi_operacao_id' => $biFilters['operacao_id'] ?? '', 'bi_page' => $i, 'map_page' => $mapPage])); ?>
            ...
            <ul class="pagination pagination-sm mb-0">
                <?php foreach ($paginationPages($colabPage, $colabTotalPages) as $i): ?>
                    <?php if ($i === null): ?>
                        ...
                        <?php continue; ?>
                    <?php endif; ?>
                    <?php $colabQuery = http_build_query(array_merge($filters, ['r' => 'relatorios/consulta', 'colab_page' => $i, 'map_page' => $mapPage, 'bi_page' => $biPage, 'voucher_page' => $voucherPage])); ?>
            ...
            <ul class="pagination pagination-sm mb-0">
                <?php foreach ($paginationPages($voucherPage, $voucherTotalPages) as $i): ?>
                    <?php if ($i === null): ?>
                        ...
                        <?php continue; ?>
                    <?php endif; ?>
                    <?php $voucherQuery = http_build_query(array_merge($filters, ['r' => 'relatorios/consulta', 'voucher_page' => $i, 'map_page' => $mapPage, 'bi_page' => $biPage, 'colab_page' => $colabPage])); ?>
