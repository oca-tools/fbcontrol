    ...
    <section class="reservation-compose-card reservation-sticky-panel">
            ...
            <form method="post" action="/?r=reservasTematicas/reservasCompleta" data-no-lock="1" data-ajax-alerts="1">
                ...
                <div class="reservation-submit-bar">
                    ...
                    <?php if ($editItem): ?>
                        <a class="btn btn-outline-primary btn-xl w-100" href="/?r=reservasTematicas/reservasCompleta">Cancelar edição</a>
                    <?php endif; ?>
                </div>
            </form>
    </section>
...
(() => {
    ...
    const fetchAvailability = async (date, forceRefresh = false) => {
        ...
        const url = `/?r=reservasTematicas/reservasCompleta&ajax=availability&data=${encodeURIComponent(date)}&_=${Date.now()}`;
    ...
    const openAvailabilityDetail = async (triggerEl) => {
        ...
        try {
            const url = `/?r=reservasTematicas/reservasCompleta&ajax=availability_detail&data=${encodeURIComponent(date)}&restaurante_id=${encodeURIComponent(restId)}&turno_id=${encodeURIComponent(turnoId)}&_=${Date.now()}`;
            const res = await fetch(url, {
                cache: 'no-store',
