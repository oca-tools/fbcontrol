...
CREATE TABLE `acessos` (
  ...
  KEY `idx_acessos_rest_oper_data` (`restaurante_id`,`operacao_id`,`criado_em`),
  KEY `idx_acessos_duplicate_scan` (`uh_id`,`operacao_id`,`criado_em`,`pax`),
...
CREATE TABLE `reservas_tematicas` (
  ...
  `correlation_item` smallint(5) unsigned NOT NULL DEFAULT 0,
  PRIMARY KEY (`id`),
  ...
  UNIQUE KEY `uq_res_tem_correlation_item` (`usuario_id`,`correlation_id`,`correlation_item`),
  KEY `idx_res_tem_data_id` (`data_reserva`,`id`),
  KEY `idx_res_tem_rest_data_id` (`restaurante_id`,`data_reserva`,`id`),
  KEY `idx_res_tem_duplicate_lookup` (`uh_id`,`data_reserva`,`turno_id`,`restaurante_id`,`status`),
